...
class GDriveSlowConfig {
  ...
  final GDriveSlowFolderMode folderMode;
  ...
  const GDriveSlowConfig({
    this.typeFolderNames = const {},
    this.extraScopes = const [],
  })  : appFolderName = null,
        folderMode = GDriveSlowFolderMode.appDataFolder;

  /// Named constructor: uses visibleFolder mode with required folder name
  const GDriveSlowConfig.visibleFolder({
    this.typeFolderNames = const {},
    this.extraScopes = const [],
    required this.appFolderName,
  }) : folderMode = GDriveSlowFolderMode.visibleFolder;
  ...
  GDriveSlowConfig copyWith({
    Map<IriTerm, String>? typeFolderNames,
    String? appFolderName,
    GDriveSlowFolderMode? folderMode,
    ...
    if (newMode == GDriveSlowFolderMode.visibleFolder && newName == null) {
      throw ArgumentError(
        'appFolderName is required when folderMode is visibleFolder',
      );
    }

    return newMode == GDriveSlowFolderMode.appDataFolder
        ? GDriveSlowConfig(
...
class GDriveTypeIndexManager {
  ...
  late final String _spaces =
      _config.folderMode == GDriveSlowFolderMode.appDataFolder
          ? 'appDataFolder'
          : 'drive';
  ...
  Future<void> _addMissingTypes({
    ...
    try {
      ...
      if (e is GDriveSlowClientException && e.message.contains('412')) {
        _log.warning('412 Conflict - Type Index was modified concurrently');
        // Fall through to conflict handling
      } else {
        rethrow;
  ...
  static GDriveSlowConfig _fillInDefaults(GDriveSlowConfig config) {
